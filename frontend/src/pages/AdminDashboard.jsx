...
import { LayoutDashboard, UserCheck, Users, FileText, FolderKanban, Flag, History } from "lucide-react";
import AppShell from "./AppShell.jsx";
import { InlineLoader } from "../components/Loader.jsx";
import { getPlatformAnalytics, listPendingReps } from "../services/adminService.js";
import { useTheme } from "../context/ThemeContext.jsx";

export function useAdminNavItems() {
  const [pendingCount, setPendingCount] = useState(0);
  useEffect(() => { listPendingReps().then((r) => setPendingCount(r.length)).catch(() => {}); }, []);
  return [
    { to: "/", label: "Overview", icon: <LayoutDashboard className="w-4 h-4" />, end: true },
    { to: "/admin/approvals", label: "Rep Approvals", icon: <UserCheck className="w-4 h-4" />, badge: pendingCount },
