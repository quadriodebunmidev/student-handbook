...
import { LayoutDashboard, Upload, FolderKanban, Plus, Pencil, X, Trash2 } from "lucide-react";
...
import { myMaterials, updateMaterial, deleteMaterial } from "../services/materialService.js";
import { useAuth } from "../context/AuthContext.jsx";
import { useToast } from "../components/Toast.jsx";

// Course Reps manage materials; Class Reps manage the course list. Both are
// stored as role "rep" (see user.repType), so the nav items and dashboard
// content branch on that instead of on a separate role.
export function repNavItemsFor(user) {
  if (user?.repType === "class") {
    return [
      { to: "/", label: "Dashboard", icon: <LayoutDashboard className="w-4 h-4" />, end: true },
      { to: "/rep/courses", label: "Manage Courses", icon: <FolderKanban className="w-4 h-4" /> },
    ];
  }
  return [
    { to: "/", label: "My Uploads", icon: <LayoutDashboard className="w-4 h-4" />, end: true },
    { to: "/rep/upload", label: "Upload Material", icon: <Upload className="w-4 h-4" /> },
    { to: "/rep/courses", label: "My Courses", icon: <FolderKanban className="w-4 h-4" /> },
  ];
}

// Kept as a plain (non-hook) export too, for pages that render before we know
// the user's repType — defaults to the Course Rep set, same as before.
export const repNavItems = [
  { to: "/", label: "My Uploads", icon: <LayoutDashboard className="w-4 h-4" />, end: true },
  { to: "/rep/upload", label: "Upload Material", icon: <Upload className="w-4 h-4" /> },
  { to: "/rep/courses", label: "My Courses", icon: <FolderKanban className="w-4 h-4" /> },
];
...
export default function RepDashboard() {
  ...
  useEffect(() => {
    ...
    myMaterials().then(setMaterials).catch(() => setMaterials([]));
  }, [user, isClassRep]);
  ...
  const totalQuizzes = materials.reduce((s, m) => s + m.quizzesGenerated, 0);
  ...
  async function handleDelete(material) {
    ...
    try {
      ...
      setMaterials((ms) => ms.filter((m) => m._id !== material._id));
  ...
  return (
    <AppShell sidebarItems={navItems}>
      ...
      <div className="mt-5 grid sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5"><p className="text-xs text-slate-500 dark:text-slate-400">Materials uploaded</p><p className="text-2xl font-bold">{materials.length}</p></div>
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5"><p className="text-xs text-slate-500 dark:text-slate-400">Total downloads</p><p className="text-2xl font-bold">{totalDownloads}</p></div>
        <div className="rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5"><p className="text-xs text-slate-500 dark:text-slate-400">Quizzes generated from your uploads</p><p className="text-2xl font-bold">{totalQuizzes}</p></div>
      </div>
